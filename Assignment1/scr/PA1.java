	...
	implements GLEventListener, KeyListener, MouseListener, MouseMotionListener
{
	...
	{  
		...
		case 2:
			...
			break;
		case 3:
			triTestPattern1(false,true); /*single color*/
			break;
		case 4:
			triTestPattern1(true,true); /*smooth shaded*/
			break;
		case 5:
			triTestPattern2(true); /*texture mapping*/
			break;
		}	
	}
	...
	public void mousePressed(MouseEvent mouse)
	{
	    ...
	    if ( button  == MouseEvent.BUTTON1 )
	    {
	    	...
	    	else if (lineSegs.size() > 0&&!Antialising)
	    		SketchBase.drawLine(buff, lineSegs.get(lineSegs.size()-2),
	    				lineSegs.get(lineSegs.size()-1));
	    	else
	    		SketchBase.drawAntiLine(buff, lineSegs.get(lineSegs.size()-2),
	    				lineSegs.get(lineSegs.size()-1));
	    }
	    ...
	    else if ( button == MouseEvent.BUTTON3 )
	    {
	    	...
	    	if (triangles.size()%3 == 1)
	    		...
	    		{
	    			if(!Antialising)  SketchBase.drawLine(buff, triangles.get(triangles.size()-2),
	    				triangles.get(triangles.size()-1));
	    			else SketchBase.drawAntiLine(buff, triangles.get(triangles.size()-2),
	    				triangles.get(triangles.size()-1));
	    		}
	    	
	    	else if (triangles.size() > 0&&!Mapping)
	    		{
	    			if(!Antialising)
	    			SketchBase.drawTriangle(buff, triangles.get(triangles.size()-3),
	    				triangles.get(triangles.size()-2), triangles.get(triangles.size()-1), doSmoothShading,Fill);
	    			else
	    				{
	    					SketchBase.drawAntiLine(buff, triangles.get(triangles.size()-3), triangles.get(triangles.size()-2));
	    					SketchBase.drawAntiLine(buff, triangles.get(triangles.size()-2), triangles.get(triangles.size()-1));
	    					SketchBase.drawAntiLine(buff, triangles.get(triangles.size()-3), triangles.get(triangles.size()-1));
	    				}// this is a little trick.. to draw the anti-aliasing triangle.....I am out of time....so....
	    		}		// I just simply connect this three point to draw a triangle without filling
	    	else
	    		SketchBase.triangleTextureMap(buff,texture, triangles.get(triangles.size()-3),
	    				triangles.get(triangles.size()-2), triangles.get(triangles.size()-1), Fill);
	    }
	...
	void lineTestPattern1()
	{
		...
		{
			SketchBase.drawAntiLine(buff, v0, v1);
			SketchBase.drawAntiLine(buff, v0, v2);
		}
	    }
	}
	  ...
	  for(petal=d_petal,theta=d_theta,i=0;i<=n_steps;++i,theta+=d_theta,petal+=d_petal)
	    {
	      ...
	      if (!Antialising) SketchBase.drawLine(buff,v0,v1);
	      else SketchBase.drawAntiLine(buff, v0, v1);
	...
	void lineTestPattern3()
	{
		...
		for(i=0,theta=0;i<n_steps;++i,theta += delta)
		{
	      ...
	      {
	    	  SketchBase.drawAntiLine(buff,tri[0],tri[1]);
	    	  SketchBase.drawAntiLine(buff,tri[1],tri[2]);
	    	  SketchBase.drawAntiLine(buff,tri[0],tri[2]);
	      }
	    }
	}
	void triTestPattern1(boolean doSmooth, boolean Fill)
	{
		float theta,delta;
		int i,cx,cy;
		float radius;
		...
		for(i=0,theta=0;i<n_steps;++i,theta += delta)
	    {
			...
			SketchBase.drawTriangle(buff,tri[0],tri[1],tri[2],doSmooth,Fill);      
	    }
	}
	
	void triTestPattern2(boolean Fill)
	{
		float theta,delta;
		int i,cx,cy;
		float radius;
		...
		for(i=0,theta=0;i<n_steps;++i,theta += delta)
	    {
			...
			SketchBase.triangleTextureMap(buff,texture,tri[0],tri[1],tri[2],true);      
	    }
	}
}